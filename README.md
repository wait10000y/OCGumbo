...
NSLog(@"[id=testId]&[href] : %@", document.Query(@"[href]").find(@"[id=testId]")); // 查找所有有href属性的标签,并且id属性内容为testId的标签.

NSLog(@"a[href=#] : %@", document.Query(@"a[href=#]")); // 查找 href属性内容为#的 所有a标签.

NSLog(@"image[alt] : %@", document.Query(@"image[alt]")); // 查找 alt属性有内容的 所有image标签.
